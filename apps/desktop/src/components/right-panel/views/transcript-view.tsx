...
export function TranscriptView() {
	...
	const handleRetranscribe = async () => {
		...
		try {
			await miscCommands.retranscribeAudio(sessionId);
			...
			toast.error("Failed to re-transcribe audio");
		} finally {
...
const SpeakerSelector = ({
	...
	const [speakerRange, setSpeakerRange] =
		useState<SpeakerChangeRange>("current");
...
function SearchAndReplace({ editorRef }: { editorRef: React.RefObject<any> }) {
	...
	useEffect(() => {
		if (editorRef.current) {
			...
			if (
				searchTerm.substring(0, searchTerm.length - 1) === replaceTerm
			) {
